...
var form = document.getElementById('form');
    ...
    form.addEventListener('submit', function(evt){
        ...
        if(cedula.value.length === 11 && cedula.value.match(num) )
        {
            ...
            {
                mensajeValido.push('este numero de cedula es valido');
            }else
            {
                mensajeError.push('este numero de cedula no es valido');
            }
        }
        else
        {
            mensajeError.push('por favor introduzca 11 digitos');
        }
